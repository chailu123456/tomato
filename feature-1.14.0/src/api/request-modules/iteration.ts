 ...
 * @returns {Promise<T>}
 */
export function getIterationEmployeeList<T>(product_id: string, iteration_id?: string | null): Promise<T> {
  const iterationId = iteration_id === "null" ? 0 : iteration_id;
  return axiosInstance.get<T>(api.getIterationEmployeeList, { product_id, iteration_id: iterationId });
}
/**
 * @param product_id 是项目id
 * @param iteration_id 迭代id
 ...
 * @returns {Promise<T>}
 */
export function getIterationDemand<T>(product_id: string, iteration_id?: string | null): Promise<T> {
  const iterationId = iteration_id === "null" ? 0 : iteration_id;
  return axiosInstance.get<T>(api.getIterationDemand, { product_id, iteration_id: iterationId });
}
