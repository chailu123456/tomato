...
const checkStaticSourceVersion = (isInit = false) => {
  ...
  axiosInstance.get(url).then((res: any) => {
    ...
    let newHash = res.data && res.data.match(/\/js\/app.(.*).js/);
    newHash = newHash ? newHash[1] : null;
...
const listenFullScreen = (cb?: (isFull: boolean) => void) => {
  const winScreenHeight = ref(window.screen.availHeight); // 屏幕高度
  let ismonted = false;
  ...
  window.addEventListener("resize", () => {
    winScreenHeight.value = window.screen.availHeight;
  });
