  ...
  <div class="container">
    <div class="main">
      ...
      <el-table :data="tableData">
        ...
        <el-table-column prop="remark" class-name="table-column-left" :autosize="{ minRows: 2, maxRows: 4 }" label="备注" maxlength="300">
          <template #default="scoped">
            <el-input placeholder="请输入0-300字" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" v-model="scoped.row.remark"></el-input>
          </template>
        </el-table-column>
    ...
    <div>
      <el-dialog title="快捷导入" v-model="exportShow" width="30%">
        <div>
          ...
          <span>
            下载测试用例模板，并按照规则填写内容
            <a href="https://file.vetscloud.com/caseTemplate.xlsx">下载模板</a>
          </span>
        </div>
        <div>
          <p>2、上传内容</p>
...
export default defineComponent({
  ...
  setup() {
    ...
    const CASE_PRIORITY_LAST = [{ value: "同上", id: 500 }].concat(priority);
    // 用例类型
    let status = JSON.parse(JSON.stringify(STATUS));

    const CASE_STATUS = status;
    const CASE_STATUS_LAST = [{ label: "同上", value: 500 }].concat(status);
        ...
        if (data && data.length) {
          ...
          for (let i = 0, l = data.length; i < l; i++) {
            if (data[i]["用例名称"]) {
              ...
              obj.level = level.length ? level[0].id : 500;
    ...
    const handleConfirmSave = () => {
      ...
      if (resultTable.length) {
        if (resultTable[0].type === "同上" || resultTable[0].type === 500) return tipMessage(400, `请选择第1条的用例类型`);
        if (resultTable[0].level === 500 || resultTable[0].level === "同上") return tipMessage(400, `请选择第1条的用例等级`);
      }
      resultTable.forEach((itemObj: Record<string, any>, index: number) => {
        ...
        itemObj.product_id = Number(getSession("productId"));
