<template>
  <div>
    ...
    >
      ...
      <template #main>
          ...
          <template #default="scope">
            <div @click.stop="">
              <el-dropdown @command="(id: string) => handleCommand(id, scope.row)" trigger="click">
                ...
                <template #dropdown>
                  <el-dropdown-menu v-for="(item, index) in tableResoult" :key="index">
        ...
        <el-table-column label="操作" width="140">
          <template #default="scoped">
            <el-button type="text" @click.stop="handleUpdateList(scoped.row)">编辑</el-button>
            <el-button type="text" @click.stop="handleRemarkList(scoped.row)">备注</el-button>
            <el-button type="text" class="delete" @click.stop="handleDeleteList(scoped.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </template>
    ...
    <el-drawer v-model="drawer" direction="rtl" :modal="false" modal-class="case-drawer" size="100%">
      <template #title>
...
export default defineComponent({
  ...
  setup() {
    ...
    const formParams = reactive<Record<string, any>>({
      ...
      product_id: Number(getSession("productId"))
    });
    ...
    const getUseCase = () => {
      ...
      getUseCaseStaff({ iteration_id: searchParams.demand }).then((res: any) => {
