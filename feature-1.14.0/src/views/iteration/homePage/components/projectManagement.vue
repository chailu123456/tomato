    ...
    <el-card class="card__employee">
      ...
      <page-table :tableData="list" :hiddenPagation="true">
        <template #main>
          ...
          <el-table-column prop="name" class-name="table-column-left" label="应用名称"> </el-table-column>
...
import useMessageTip from "@/composables/useMessageTip";
import { defineComponent, onBeforeUpdate, PropType, watch } from "vue";
import { ref } from "vue";
import { RequestParams } from "@/types/request";
import { searchParams } from "../../useMixin";
import { Plus, CirclePlus, Remove } from "@element-plus/icons";
...
export default defineComponent({
  ...
  components: { Plus, CirclePlus, Remove },
  emits: ["refresh"],
  setup(props, { emit }) {
    const rules = {
          ...
          formRefs.value[i].validate((valid: boolean) => {
            ...
            if (i === formRefs.value.length - 1) {
              if (flag.includes(false)) {
                ...
                //  提交表单
                // const iteration_id = Number((route.query as Record<string, any>).id);
                const params = {
