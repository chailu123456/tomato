...
export default function useGetEmployeeAndAppLIsts(
  ...
  const handleConfirmSubmit = () => {
    ...
    updateEmployeeList<ResponseParams.ResponseDataSuccess>({
      ...
      tipMessage(res.code);
      emit("refresh", searchParams.demand);
    });
  ...
  const dataReverse = (people: Array<Record<string, any>>) => {
    ...
    for (let i = 0; i < people.length; i += 1) {
      ...
      obj.disabled = people[i].is_department ? true : false;
  ...
  //#region 获取部门组织和员工
  const getMembers = () => {
    getProductMember<ResponseParams.ResponseDataSuccess>({
      product_id: Number(getSession("productId")),
      iteration_id: searchParams.demand
    }).then((res) => {
      // 遍历第一级数据，第一级数据son为空的话，去请求当前下Node的员工列表
      // function dpsNode(arr: Array<Record<string, any>>) {
      //   arr.forEach((v: Record<string, any>) => {
      //     if (v.son.children) {
      //       v.uniqueId = v.id;
      //       dpsNode(v.son);
      //     } else {
      ...
      // (res.data as Array<Record<string, any>>).forEach((v) => {
      //   v.son.forEach((item: Record<string, any>) => {
      //     item.uniqueId = item.id;
      //     callbacks.push(...getEmployee(item));
      //   });
      // });
      // run(callbacks);
      deparment.value = dataReverse(res.data);
    });
  };
  getMembers();
