    ...
    <div class="pad-top" v-show="isShareSidebar">
      ...
      <div class="pad-top__tools">
        ...
        <el-dropdown v-if="status !== 2">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item @click="handleMenuItem(item.id)" :disabled="item.id > 2 && permission !== 2" v-for="item in dropList" :key="item.id">
      ...
      <el-form :model="formParams" label-width="100px" ref="shareFormRef">
        <ul class="rp-share-select">
          <li v-if="!hasPermissOpt" class="rp-share-select__column">
            <ShareClassify :classify_id="formParams.classify_id" @change="onChangeSelect" :trees="columnShareList" />
          </li>
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleMenuItem = async (id: number) => {
      switch (id) {
        case 0:
          // 新窗口打开
