      ...
      <template #main>
        <el-table-column prop="name" class-name="table-column-left table-title" label="名称">
          <template #default="scope">
            <span class="project-status" :style="statusType(scope.row.type, 1)">{{ statusType(scope.row.type, 0) }}</span>
            <span class="project-title" :style="{ color: scope.row.status === 3 ? 'rgb(160, 160, 160)' : '#333' }" @click="handleBtn(scope.row)">{{
              scope.row.name
            }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="role" width="280" label="时间">
          <template #default="scope">
