    ...
    case 4: // 任务详情
      {
        ...
        if (productId.value === props.item?.product_id) {
          ...
          if (props.item?.product_id) {
            ...
            setSession("productId", props.item?.product_id.toString());
        ...
        if (props.item?.iteration_id) setCache(props.item?.iteration_id);
        router.push({
          name: "exploitation",
          query: {
    ...
    case 6: // 迭代详情
      {
        ...
        if (productId.value === props.item?.product.id) {
          ...
          if (props.item?.product.id) {
            ...
            setSession("productId", props.item?.product?.id.toString());
        ...
        if (props.item?.id) setCache(props.item?.id);
    ...
    case 8: // bug详情
      {
        router.push({
          name: "bugDetail",
          query: { bugId: props.item?.id, product_id: props.item?.product_id }
        });
      }
      break;
  }
