<template>
  <el-header class="header">
    ...
    <section class="header-right">
      <div>
        <el-button v-if="!isDocument" type="primary" @click="handleShowDialog">新增项目</el-button>
        <el-button v-if="isDocument === '文档管理'" type="primary" @click="handleAddDoc">新增文档</el-button>
      </div>
      <!-- 消息通知模块 start -->
      <div class="side-wrapper__notice" @click="show">
        <el-popover placement="bottom-end" trigger="manual" :width="900" v-model:visible="visible">
          <template #reference>
            <div>
              <el-badge :value="notices" :hidden="notices <= 0 ? true : false" type="danger" :max="99">
                <!-- <i id="icon" class="el-icon-bell side-wrapper__notice-icon"></i> -->
      ...
      <div class="header-pageName__profiles">
        <el-popover popper-class="header-pageName__profiles--popover" placement="bottom-end" :width="150" trigger="hover">
          <template #reference>
            <img class="logo-wrapper__collapse" :src="user.avatar ? user.avatar : '../../assets/collapse-logo.png'" alt="" />
          </template>
          <div>
            ...
            <div style="margin: 6px 0">
              <el-switch style="margin-bottom: 6px" v-model="bug_alert" @change="handleTip" inactive-text="bug企微提醒"></el-switch>
              <el-switch v-model="task_alert" @change="handleTip" inactive-text="任务企微提醒"></el-switch>
            </div>
...
import { clearSession, getDomain } from "@/utils";
...
export default defineComponent({
  ...
  setup() {
    ...
    const handleAddDoc = () => {
      ...
      //   query: {
      //     type: "update",
      //     origin: getSession("defaultActiveTab") as string
      //   }
      // });
    };
    ...
    const getDemandListFn = (id: number) => {
      ...
      getDemandList(id).then((res) => {
        ...
        // 获取迭代详情
        // res.length && getIterationDetail(res[0].id);
      });
    };
    ...
    const handleChangeProduct = (id: number) => {
      setSession("productId", id.toString());
      const item = productLists.value.find((i: Record<string, any>) => i.id === id);
      setSession("productName", item?.name);
    ...
    const getProductLists = () => {
      selectProductList<ResponseParams.ResponseDataSuccess>().then((res) => {
        ...
        const localProductId = getSession("productId");
