...
export function replaceProductId(): Record<string, any> {
  function checkProductId(lists: Array<Record<string, any>>, id: string) {
    ...
    if (index >= 0) {
      ...
      // 这块是进行对项目一级目录下迭代模块中任务、bug、用例链接中带iterateId进行判断最后更新迭代
      if (location.href.includes("project/iteration")) {
        setTimeout(() => {
          const { iterateId } = router.currentRoute.value.query;
          console.log(iterateId);
          const iterateList = computed(() => store.getters.iterateList);
          // console.log(iterateList.value);
          const curInter = iterateList.value.filter((item: any) => item.id === Number(iterateId));
          // console.log(curInter);
          if (curInter && curInter.length) {
            store.commit(MutationType.updateIterateId, iterateId);
            // store.commit(MutationType.updateIterateList, curInter);
            store.commit(MutationType.updateCurrentIter);
          } else {
            if (!location.href.includes("project/iteration/homepage")) {
              // if (Number(iterateId) == 0) window.location.href = "/noAccess";
              console.log(iterateId + "11000");
              // if (iterateId) window.location.href = "/noAccess";
            }

            if (location.href.includes("project/iteration/homepage") && iterateId) window.location.href = "/noAccess";
            // if (!iterateId) window.location.href = "/noAccess";
          }
          // console.log(curInter);
        }, 500);
      ...
      //   const { detailId } = router.currentRoute.value.query;
      //   if (detailId) {
      //     // 访问无权限的详情页面时
      //     // window.location.href = "/noAccess";
      //   } else {
  ...
  if (!routerProductId) {
    ...
    const storgeProductId = productId.value;
    if (!storgeProductId) {
      //  缓存，地址栏都没有,使用默认的第一个
      if (productLists.value.length) {
        // document表示在文档模块不需要添加项目id到地址栏
        if (!router.currentRoute.value.meta.excludeProductId) {
          router.replace({
            query: Object.assign(
              { ...router.currentRoute.value.query },
              {
        ...
        watch(
          () => productLists.value,
          (newValue) => {
            if (newValue && newValue[0]?.id) setSessionFn(newValue[0].id);
          }
        );
      }
    } else {
      ...
      const { iterateId } = router.currentRoute.value.query;
      let curIterId: unknown = undefined;
      if (iterateId) {
        curIterId = iterateId;
      } else {
        curIterId = store.getters?.iterateId || undefined;
      }
      if (!router.currentRoute.value.meta.excludeProductId) {
        setTimeout(() => {
          router.replace({
            query: Object.assign(
              { ...router.currentRoute.value.query },
              {
                ...
                iterateId: router.currentRoute.value.meta.showIteration ? curIterId : undefined
              }
            )
          });
        }, 200);
      } else {
        if (location.href.includes("lookBoard/monthOverview")) {
          router.replace({
            query: Object.assign({
              iterateId: undefined
            })
    ...
    const { iterateId } = router.currentRoute.value.query;
    if (iterateId) {
      getCurId.value = iterateId;
    } else {
      getCurId.value = store.getters?.iterateId || undefined;
    }
    setTimeout(() => {
      router.replace({
        query: Object.assign(
          { ...router.currentRoute.value.query },
          {
            productId: routerProductId,
            iterateId: router.currentRoute.value.meta.showIteration ? getCurId.value : undefined,
            come: undefined
          }
        )
      });
