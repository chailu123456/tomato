<template>
  <div v-if="hasData" class="rp-test">
    <page-table
      :tableData="list"
      :currentPage="page"
      :total="total"
    ...
    >
      ...
      <template #main>
        ...
        <el-table-column sortable="custom" prop="name" class-name="table-column-left table-title" min-width="200" label="Bug标题" #default="scope">
          <span class="rp-Table__bug" @click="handleShowDetail(scope.row.id)">{{ scope.row.name }}</span>
        </el-table-column>
        <!-- 状态 -->
    ...
    <div>
      ...
      >
        <template #form>
          <form-bug :drawerForm="drawerForm" :staffList="employeeLists" :currIterateId="iterateId" @onHandle="handleBugDetail"></form-bug>
        </template>
      </Drawer>
    </div>
    <!-- 指派人弹出框 -->
...
// 不要删掉，应对全屏名称的样式问题
:global(.rp-test .table-column-left) {
  font-size: 14px;
}
</style>
