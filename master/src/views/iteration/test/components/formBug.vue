  ...
  <div class="rp-form-content">
    <el-form :model="drawerForm" ref="ruleFormRef" label-position="left" label-width="100px">
      ...
      <el-form-item label="所属项目" prop="product_id" required>
        <el-select
          @change="(value: string) => handleChangeForm(getIdByName(value, productList), 'product_id')"
          :popperAppendToBody="false"
          class="reset-select"
          filterable
          v-model="drawerForm.product_name"
          placeholder="请选择项目"
          :disabled="currentRoute.meta.inIteration"
        >
          <el-option v-for="it in productList" :key="it.id" :label="it.name" :value="it.name"></el-option>
        </el-select>
      </el-form-item>

      <el-form-item label="所属迭代" prop="iteration_id">
        <el-select
          @change="(value: any) => handleChangeForm(getIdByName(value, iterationList), 'iteration_id')"
          :popperAppendToBody="false"
          class="reset-select"
          filterable
          clearable
          v-model="drawerForm.iteration_name"
          placeholder="请选择迭代"
          :disabled="currentRoute.meta.inIteration"
        >
          <el-option v-for="it in iterationList" :key="it.id" :label="it.name" :value="it.name"></el-option>
        </el-select>
      </el-form-item>

      <el-form-item label="关联需求" prop="demand_id">
        <el-select
          @change="(value: any) => handleChangeForm(getIdByName(value, demandList), 'demand_id')"
          :popperAppendToBody="false"
          class="reset-select"
          filterable
          clearable
          v-model="drawerForm.demand_name"
          placeholder="请选择需求"
        >
          <el-option v-for="it in demandList" :key="it.id" :label="it.name" :value="it.name"></el-option>
        </el-select>
      </el-form-item>

      <el-form-item label="关联任务" prop="task_id">
        <el-select
          @change="(value: string) => handleChangeForm(getIdByName(value, taskList), 'task_id')"
          :popperAppendToBody="false"
          class="reset-select"
          filterable
          clearable
          v-model="drawerForm.task_name"
          placeholder="请选择任务"
        >
          <el-option v-for="it in taskList" :key="it.id" :label="it.name" :value="it.name"></el-option>
        </el-select>
      </el-form-item>

      <el-form-item label="关联用例" prop="test_case_id">
        <el-select
          @change="(value: any) => handleChangeForm(getIdByName(value, useCaseList), 'test_case_id')"
          :popperAppendToBody="false"
          class="reset-select"
          filterable
          clearable
          v-model="drawerForm.test_case_name"
          placeholder="请选择用例"
        >
          <el-option v-for="it in useCaseList" :key="it.id" :label="it.name" :value="it.name"></el-option>
        </el-select>
      </el-form-item>
...
const getBugTaskData = (params: number) => {
  let iterationId = 0;
  if (!props.drawerForm.id && inIteration.value) {
    iterationId = props.currIterateId;
  }
  const obj = {
    product_id: params,
...
const getIdByName = (name: string, list: SelectItem[]) => {
  const target = find(list, (o) => name === o.name);
  return target?.id;
};

// 更新表单数据 props.demandForm.id 存在代表编辑，不存在代表创建。
const handleChangeForm = async (val: any, key: string) => {
  props.drawerForm[key] = val;
  // 如果是切换项目，则需要更新项目下的数据
  if (key === "product_id") {
...
watch(
  ...
  throttle((newVal) => {
    dealStaffList();
    getIterationData(newVal.product_id);
    getNewDemandData(newVal.product_id);
    getBugTaskData(newVal.product_id);
    getUseCaseData(newVal.product_id);
  }, 300),
  {
    immediate: true
    // deep: true
