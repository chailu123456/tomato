    ...
    <el-form ref="ruleFormRef" :model="form" label-position="left" label-width="60px">
      ...
      <div v-show="reactiveItem.showMore">
        ...
        <el-form-item prop="start_time_min" class="wait-form-time" label="开始">
          ...
          <el-time-select v-model="form.start_time_min" size="default" start="00:00" step="00:05" end="24:00" placeholder="请选择时间" />
        </el-form-item>
...
watch(
  ...
  (newVal) => {
    if (newVal && Object.keys(newVal).length) {
      ...
      form.remark = "";
      form.duration = 1;
      form.ahead_time_enum = 2;
      form.is_repeat = 0;
      form.outName = "";
      form.filter_holiday = 0;
      form.department = [];
      form.participant = [];
    }
  },
...
const updateCurDate = () => {
  const hours = dayjs().format("HH:mm");
  // 更新开始结束时间
  updateTime(`${props.personInfo?.day} ${hours}`);
};
