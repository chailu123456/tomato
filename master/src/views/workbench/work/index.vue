<template>
  <el-calendar v-loading="loading" ref="calendar" class="work-calendar work-calendar-liner" id="work-calendar" v-model="calendarVal">
    <template #dateCell="{ data }">
      <CalendarSlot :calendarList="list" :calHeight="calHeight" @lookDetail="lookDetail" @updateData="getList" :data="data" />
    </template>
  </el-calendar>
</template>
...
import { computed, defineExpose, onMounted, defineProps, watch, nextTick, ref, defineEmits } from "vue";
import CalendarSlot from "./calendar-slot.vue";
import { useStore } from "@/store/index";
import { MutationType } from "@/store/mutation-types";
import dayjs from "dayjs";
import { throttle } from "lodash";
import type { CalendarItemInter } from "@/composables/useDashboard";
...
const getList = async () => {
  ...
  loading.value = true;
  const { name, status, type, end_time, start_time } = props.formParams;
  nextTick(async () => {
    const selectedDay = calendar.value.selectedDay;
    if (selectedDay) {
      await useGetCarlenderInfo({ month: selectedDay.format("YYYY-MM"), name, status, type, end_time, start_time });
    }
    loading.value = false;
  });
};
