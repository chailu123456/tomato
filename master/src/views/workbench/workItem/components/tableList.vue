    ...
    >
      <template #main>
        ...
        <el-table-column prop="role" width="280" label="预计完成时间">
          <template #default="scope">
            <span :style="stypeStatus(scope.row)"> {{ scope.row.end_time ? dayjs(scope.row.end_time).format("YYYY-MM-DD HH:mm") : "" }}</span>
          </template>
        </el-table-column>
      </template>
    </page-table>
...
<script lang="ts">
import { ref, defineProps, PropType, watch, reactive, defineEmits, nextTick } from "vue";
export default {
  name: "tableList"
};
</script>
...
const handleConditionSearch = async () => {
  ...
  timer = setTimeout(async () => {
    if (props.formParams?.is_drawer) {
      ...
      await getData({ pageIndex: 1, pageSize: pageTableRef.value?.getCurrentPage().pageSize || 20 }, true, props.formParams);
    }
