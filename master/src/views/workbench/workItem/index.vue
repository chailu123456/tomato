<template>
  <div class="rp-content-workItem">
    ...
    <WorkDrawer :type="typeDrawer" :targetId="targetId" ref="workDrawer" @refresh="refreshPanel"></WorkDrawer>
  </div>
</template>

<script lang="ts">
import { ref, reactive, defineAsyncComponent, markRaw, onActivated, nextTick } from "vue";
...
const componentArr = [
  ...
  markRaw(defineAsyncComponent(() => import("../dashboard/components/panelTimeline.vue")))
];
const visible = ref(false);
const personVisible = ref(false);
// 工作项视图菜单
const switchMenu = ["列表", "日历", "TODO", "时间轴"];
...
onActivated(() => {
  ...
  if (route.params && route.params.where === "dashboard") {
    ...
    searchArray.searchForm[2].val = Number(route.params.type) ? [6] : [1, 2];
    formParams.type = [2, 5];
    formParams.status = Number(route.params.type) ? [6] : [1, 2];
    nextTick(() => {
      handleSwitch(0);
    });
    ...
    nextTick(() => {
      ...
      formParams.is_drawer = 0;
      handleSwitch(0);
    });
  } else {
    currentActive.value = "-1";
    searchArray.searchForm[1].val = [];
    searchArray.searchForm[2].val = [];
    formParams.status = [];
    formParams.type = [];
    refreshPanel();
  }
...
VueEvent.on("drawStatus", (val) => {
  if (route.path === "/workbench/workItem") {
    if (!val) {
      nextTick(() => {
