...
import { removeIdFromRouter, addDetailIdToRouter } from "@/utils/index";
...
import { reactive, computed, defineProps, defineEmits, ref, onMounted, unref, defineExpose, watch } from "vue";
...
watch(
  ...
  () => {
    ...
    if (router.currentRoute.value.name === "testBill" && detailId) {
      showEditDrawer(Number(detailId));
    }
  },
  {
    deep: true
...
onMounted(() => {
  const { detailId } = route.query as Record<string, string>;
  if (router.currentRoute.value.name === "testBill" && detailId) {
    showEditDrawer(Number(detailId));
  }
});
...
const showEditDrawer = async (row: ProductTestBillItem | number) => {
  ...
  !currentRt.form.length && getUserList();
};
