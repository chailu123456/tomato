<template>
  <div class="rp-setting-person">
    <el-form :model="dialogParams" :rules="rules" ref="formRef" label-width="120px" class="product-form">
      ...
      <el-form-item label="周会汇报" prop="week_report">
        <!-- TODO 需要汇报需要把团队名称也加上 -->
        <p v-if="dialogParams.week_report === 1" style="margin: 0">
          需要汇报 <span style="margin-left: 5px; color: #999">（需要在部门周会中汇报）， {{ dialogParams.team_name }}团队</span>
        </p>
        <p v-else style="margin: 0">无需汇报 <span style="margin-left: 5px; color: #999">（不需要在部门周会中汇报）</span></p>
      </el-form-item>
