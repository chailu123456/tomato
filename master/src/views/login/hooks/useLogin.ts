 ...
 * @LastEditTime: 2022-07-25 15:01:33
 * @LastEditors: 宋绍华
 * @Description:
  ...
  const getQRCode = async () => {
    ...
    if (code) {
      ...
      try {
        ...
        // 判断缓存中有jumpUrl
        if (getSession("redirectUrl") && getSession("redirectUrl") !== "/login") {
          const url = getSession("redirectUrl") as string;
          location.href = url;
          clearSession("redirectUrl");
