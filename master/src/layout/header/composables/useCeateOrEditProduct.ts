...
export default function useCeateOrEditProduct(): any {
  ...
  const productRef = ref<
    (HTMLElement & { resetFields: () => void; clearValidate: () => void; validate: (...args: Array<any>) => void; validateField: (props: any) => void }) | null
  >(null);
  ...
  const confirmSave = async (
    ref: HTMLElement & { resetFields: () => void; clearValidate: () => void; validate: (...args: Array<any>) => void; validateField: (props: any) => void },
    formData?: any,
    callback?: () => void
  ) => {
    if (ref) {
    ...
    if (formData) {
      if (formData?.week_report === 1 && !formData?.team_id) return ElMessage.warning("请选择团队");
      Object.keys(dialogParams).forEach((key) => {
        // @ts-ignore
        dialogParams[key] = formData[key];
      });
    } else {
      if (dialogParams?.week_report === 1 && !dialogParams?.team_id) return ElMessage.warning("请选择团队");
    }
      ...
      if (valid) {
        ...
        selectProductList<ResponseParams.ResponseDataSuccess>().then((res: Record<string, any>) => {
          ...
          if (!formData) {
            ...
            store.commit(MutationType.updateProductId, productId.value);
            // 设置项目缓存
            store.commit(MutationType.updateProductInfo, productLists.value[0]);
            // 新建项目清空迭代缓存
