 ...
 * @LastEditTime: 2022-08-18 12:06:30
 * @LastEditors: 宋绍华
 * @Description:
...
<template>
  <div class="projectNav">
    ...
    <template v-if="!currentRouteMeta.showProductCollect">
    ...
    <!-- 项目集 -->
    <span v-if="currentRouteMeta.showProductCollect">></span>
    <ProductCollect
      v-if="currentRouteMeta.showProductCollect"
      :prop-val="boardBaseRt.currentGroupIds"
      :list="boardBaseRt.teamProductList"
      @on-change="onProductChange"
      class="projectNav-product-collect reset-select"
      :class="{ 'projectNav-product-collect2': boardBaseRt.currentGroupIds.length > 1 }"
    />
  </div>
</template>
...
const onSelect = async (type: ProjectNavSearchType, data: Iterate | ProjectItem) => {
  ...
  if (type === "iteration") {
    ...
    setTimeout(() => {
      if (currentRoute.value.includes("/project/product")) {
        router.replace({
          query: Object.assign(
            { ...router.currentRoute.value.query },
            {
        ...
        // 不在显示迭代id的地方，不需要带迭代id
        // const _iterateId = currentRouteMeta.value?.showIteration ? iterateId.value || undefined : undefined;
        router.replace({
          query: Object.assign(
            { ...router.currentRoute.value.query },
            {
