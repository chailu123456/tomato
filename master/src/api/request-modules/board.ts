 ...
 * @LastEditTime: 2022-07-12 16:43:10
 * @LastEditors: 宋绍华
 * @Description:
 ...
 * @returns
 */
export function getIterateList<T>(params?: { product_id?: number; iteration_id?: number | null; is_bug?: number }): Promise<T> {
 ...
 * @returns
 */
export function getTeamList<T>(): Promise<T> {
  return axiosInstance.get<T>(api.teamList);
}
 ...
 * @returns
 */
export function getTeamProductList<T>(): Promise<T> {
  return axiosInstance.get<T>(api.teamProductList);
}
