...
import { AssociatedReq } from "@/composables/useHomePage";
/**
 *
 * @param status  状态
...
export function getTaskList<T>(params: Record<string, any>): Promise<T> {
  const timeKeyArr = ["expected_start_time", "expected_end_time", "actual_start_time", "actual_end_time"];
  for (let i = 0; i < timeKeyArr.length; i++) {
    if (params[timeKeyArr[i]] && !params[timeKeyArr[i]].length) delete params[timeKeyArr[i]];
    if (!params[timeKeyArr[i]]) delete params[timeKeyArr[i]];
  }
  delete params.times;
  delete params.time;
 ...
 * @returns
 */
export function getDocList<T>(params: { iteration_id: number }): Promise<T> {
  return axiosInstance.get<T>(api.docList, params);
}
interface Docs {
  id: number;
  title: string;
 ...
 * @returns
 */
export function updateUi<T>(params: DocUi): Promise<T> {
  return axiosInstance.put<T>(api.updateUi, params);
}
 ...
 * @returns
 */
export function addUi<T>(params: DocUi): Promise<T> {
  return axiosInstance.post<T>(api.addUi, params);
}

/**
 * @description  迭代移除关联文档
 * @returns {Promise<T>}
 */
export function removeDocRelative<T>(params: { doc_id: number; iteration_id: number }): Promise<T> {
  return axiosInstance.delete<T>(api.removeDocRelative, { data: params });
}
 ...
 * @returns
 */
export function interDocTelativetree<T>(params: { iteration_id: number }): Promise<T> {
  return axiosInstance.get<T>(api.interDocTelativetree, params);
}
 ...
 * @returns
 */
export function updateDocRelative<T>(params: { iteration_id: number; doc_ids: number[] }): Promise<T> {
  return axiosInstance.post<T>(api.updateDocRelative, params);
}
...
export function getUseCaseData<T>(params: RequestParams.GetUseCaseList): Promise<T> {
  ...
  const arr = ["create_by", "type", "status", "level", "staff_no"];
  arr.forEach((item: string) => {
    if (!obj[item]) delete obj[item];
  });

  return axiosInstance.get<T>(api.getUseCaseData, obj);
}
 ...
 * @returns {Promise<T>}
 */
export function useCaseDetail<T>(id: number): Promise<T> {
  return axiosInstance.get<T>(api.useCaseDetail + "/" + id);
}
 ...
 * @returns {Promise<T>}
 */
export function downloadTemplate<T>(params: { product_id: number }): Promise<T> {
  return axiosInstance.get<T>(api.useCaseTemplate, params);
}
 ...
 * @returns {Promise<T>}
 */
export function useCaseImport<T>(params: any): Promise<T> {
  return axiosInstance.post<T>(api.useCaseImport, params);
}
 ...
 * @returns {Promise<T>}
 */
export function getCaseLog<T>(id: number): Promise<T> {
  return axiosInstance.get<T>(`${api.useCaseLog + "/" + id + "/logs"}`);
}
 ...
 * @description  计划关联的需求和需求文档
 * @returns {Promise<T>}
 */
export function getAssociatedDemandDoc<T>(params: { plan_id: number; iteration_id: number }): Promise<T> {
  return axiosInstance.get<T>(`${api.getPlanLists}/${params.plan_id}/associated-demand-doc?iteration_id=${params.iteration_id}`);
}
/**
 * @description  迭代关联计划
 * @returns {Promise<T>}
 */
export function updateIterPlan<T>(params: AssociatedReq): Promise<T> {
  return axiosInstance.put<T>(api.updateIterPlan, params);
}
