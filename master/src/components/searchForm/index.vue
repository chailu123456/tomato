            ...
            <el-form ref="formRef" label-width="80px" class="form-search-list">
              ...
              >
                ...
                <template v-else-if="item.type === 'daterange'">
                  <el-select class="timeType" v-if="item.typeTime" :teleported="false" @change="handleTime" v-model="item.typeTime">
...
import { ref, defineProps, defineEmits, watch, markRaw, defineAsyncComponent, defineExpose, onActivated } from "vue";
...
const componentArr = [
  ...
  markRaw(defineAsyncComponent(() => import("@/businessComponents/selectTree/index.vue")))
];
...
const keys = ["expected_start_time", "expected_end_time", "actual_start_time", "actual_end_time"];
...
const handleReset = (formEl: FormInstance | undefined) => {
  ...
  props.searchArray.searchForm.forEach((item: any) => {
    item.val = "";
    if (item.type === "customComponents" && item.componentIndex === 2) {
      cascaderVal.value?.handleClearTree("delete");
    }
    if (item.type === "daterange" && item.typeTime) {
      item.typeTime = 1;
      item.key = "expected_start_time";
    }
  });
...
const handleCancel = () => {
  ...
  props.searchArray.searchForm.forEach((item: SearchForm, index: number) => {
    ...
    // 处理用例模块时间
    if (item.type === "daterange" && item.typeTime) {
      item.typeTime = oldData[index].typeTime;
      item.key = oldData[index].key;
    }
    ...
    if (item.type === "customComponents") {
      ...
      if (item.type === "customComponents" && item.componentIndex === 2) {
        cascaderVal.value?.confirmSearch(item.val);
      }
    }
  });
