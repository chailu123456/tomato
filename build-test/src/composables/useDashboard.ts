...
import { selectProductList, getDemandDetail } from "@/api/request-modules/product";
import { getBugDetail } from "@/api/request-modules/test";
import {
  delCalenderBackLogApi,
  getBackLogDetailApi,
  getCalendarListApi,
  getTomatoBackLogList,
  handleBackLogApi,
  handleBackLogStatusApi,
  updateBugStatusApi
} from "@/api/request-modules/workbench";
import { awaitFunc, setSession, updateProjectCache } from "@/utils";
import { ref } from "vue";
...
export interface BackLogListItemIter {
  id: number; // 待办id
  name: string; // 待办名称
...
export default function useDashboard() {
  ...
  const useGetDemandDetail = async (params: { id: number }): Promise<number | null> => {
    const [err, data]: [unknown, any] = await awaitFunc<{ id: number }, any>({
      asyncFunc: getDemandDetail,
      args: params
    });
    if (err || !data) return null;
    return data;
  ...
  const useHandleProductCache = async (productId: number, hasList = false) => {
    ...
    if (item) {
      // 设置项目缓存
      updateProjectCache(item as any);
      return true;
    }
    return false;
  };
  ...
  // 获取待办列表
  const useGetBackLogList = async (params: { date: string }): Promise<BackLogListItemIter[] | null> => {
    const [err, data]: [unknown, BackLogListItemIter[] | null] = await awaitFunc<{ date: string }, BackLogListItemIter[]>({
      asyncFunc: getTomatoBackLogList,
      args: params
    });
    if (err || !data) return null;
    return data;
