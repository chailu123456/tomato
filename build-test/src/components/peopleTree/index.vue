<template>
  <div>
    <el-dialog title="添加" custom-class="position-dialog" v-model="dialogShadow" :append-to-body="true" width="60%">
      ...
      <div>
        <el-scrollbar height="400px">
          <div v-for="(item, index) in selectPerson" :key="index" class="current-checked-nodes">
            ...
            <el-icon style="margin-right: 10px" v-if="showRemoveIcon" class="el-icon-close" @click="deleteCheckedKeys(item, index)">
              <Close />
            </el-icon>
          </div>
        </el-scrollbar>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const handleShadow = async () => {
      console.log(selectPerson.value[0]);
      emit("personMsg", selectPerson.value[0], props.type);
