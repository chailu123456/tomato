 ...
 * @LastEditTime: 2022-06-10 11:22:41
 * @LastEditors: 宋绍华
 * @Description:
...
<template>
  <div class="testBill">
    ...
    <div class="testBill-search">
      ...
      <el-button @click="addTestBill" class="js-drawer-uncloseable" type="primary">新增提测单</el-button>
    </div>
    <!-- table -->
    <page-table
    ...
    >
      <template #main>
        <el-table-column v-for="item in currentRt.tableColumn" :key="item.prop" :label="item.label" :prop="item.prop" :min-width="item.minWidth">
          <template #default="scoped">
            ...
            <div v-else-if="item.prop === 'name'">
              <span @click="showEditDrawer(scoped.row)" style="text-align: left; display: block" class="table-link js-drawer-uncloseable">{{
    ...
    />
    <HandleTestBill @update-info="getTestBillList(currentRt.searchForm!)" ref="testBillComp" />
  </div>
</template>
...
interface TestBill {
  ...
  tableColumn: { prop: string; label: string; minWidth?: number }[];
...
// 获取列表数据
const getTestBillList = async (params: ProductTestBillReq) => {
