      ...
      <template #header v-if="comePlan != 'comePlan'">
        <div class="search-header">
          ...
          <div class="search-right">
            ...
            <el-button class="js-drawer-uncloseable" type="primary" @click="handleAddDemand">新 增</el-button>
          </div>
        </div>
      </template>
      <template v-if="isShow" #main>
        ...
        <el-table-column sortable="custom" class-name="table-column-left table-title" :min-width="200" prop="name" label="需求名称">
          <template #default="scoped">
            <span class="js-drawer-uncloseable app-name" @click="handleDetail(scoped.row)">{{ scoped.row.name }}</span>
          </template>
        </el-table-column>
            ...
            <div class="demand-detail-show" v-infinite-scroll="() => handleLoad(item)" style="overflow: auto">
              <div>
                <div
                  ...
                  :key="index"
                >
                  <p style="word-break: break-all; word-wrap: break-word" @click="handleDetail(it)">{{ it.name }}</p>
...
<script lang="ts">
import { defineComponent, toRefs, onMounted, reactive, onDeactivated, ref, watch, onActivated } from "vue";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    watch(
      () => drawer.value,
      (newVal) => {
        if (!newVal) getData(pageTableRef.value.getCurrentPage(), undefined, searchParams.value);
      }
    );
    ...
    const getdemandDetailFn = (id: number) => {
      getDemandDetail<ResponseParams.ResponseDataSuccess>({ id: id })
        .then((res) => {
    ...
    const handleAddDemand = () => {
      ...
      console.log(JSON.parse(JSON.stringify(demandForm.value)));
    ...
    onMounted(() => {
      // const { productId, type } = route.query as Record<string, any>;
      // if (status) {
      //   searchParams.value.demand_status = status?.split(",").map((i: string) => parseInt(i));
      //   router.replace({
      //     path: route.path,
      //     query: { productId }
      //   });
    ...
    useWatchChangeProduct(handleConditionSearch, async (newValue) => {
      ...
      getModuleList<ResponseParams.ResponseDataSuccess>({ product_id: Number(newValue) }).then((res) => {
        if (res.data && res.data.length) {
          ...
          basicModuleList.value = moduleList.value.map((o: Record<string, any>) => {
            return { value: o.id, label: o.name };
          });
        } else {
    ...
    const searchDate = async () => {
      pageTableRef.value?.setCurrentPage();
      currentPage.value = 1;
      await getData({ pageIndex: 1, pageSize: 20 }, undefined, Object.assign(searchParams.value));
      flag.value = false;
    };
    ...
    //基础操作列表数据
    const basicsOption: any = reactive([
      {
        value: "level",
        label: "优先级",
