<template>
  <div class="overview" id="overview">
    ...
    >
      ...
      <template #main>
        <el-table-column prop="version_name" sortable="custom" label="迭代版本号 - 名称" fixed="left" class-name="table-column-left" min-width="140">
  ...
  setup(props, { emit }) {
    ...
    const getSearchData = () => {
      ...
      getIterationDemand<ResponseParams.ResponseDataSuccess>({
        ...
        if (res.data && res.data.length) {
          newDemandLists.value = res.data.map((o: Record<string, any>) => {
    ...
    onMounted(() => {
      ...
      if (month) {
        ...
        searchParams.value.product_id = product_id ? parseInt(product_id) : null;
    ...
    watch(
      () => route.query,
      () => {
        const { where, productId } = route.query as Record<string, string>;
        if (where) {
