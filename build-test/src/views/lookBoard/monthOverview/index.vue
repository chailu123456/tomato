        ...
        <div class="overflow-title">
          ...
          <span class="right-jump">
            <router-link
              ...
              :to="`/lookBoard/personal/jobEvaluation?productId=${searchCriteria.product_id}&month=${searchCriteria.month}`"
...
import { updateProjectCache } from "@/utils";
type SearchCriteria = {
  month: string;
  product_id: number;
};
...
watch(
  ...
  (newVal) => {
    if (newVal) {
      searchCriteria.product_id = newVal;
      getData();
    }
  }
);
...
const handleChangeProduct = (id: number) => {
  ...
  const item = productList.value.find((item: ProjectItem) => item.id === id);
  if (item) {
    // 设置项目缓存
