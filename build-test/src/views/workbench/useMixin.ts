...
export async function updateInterId(item: Record<string, any>, module?: string) {
  ...
  const routeMap = {
    projectExploitation: {
      ...
      query: { iteration_id: item.iteration_id, taskId: item.target_id, name: item.name, where: "workbench" }
    },
    homepage: {
      baseName: "homepage",
      query: {
...
export function dynamicsInterId(item: Record<string, any>) {
  ...
  const routeMap = {
    ...
    projectExploitation: {
      baseName: "projectExploitation",
      query: { iteration_id: item.iteration_id, taskId: item.obj_id, name: item.obj_name, where: "workbench" }
    },
    test: {
      baseName: "test",
      query: {
